...
public:
     ...
     GLushort id;
     GLushort rot_around_id;
     vector<glm::vec4> vertices;
     vector<glm::vec3> normals;
     vector<GLushort> elements;
     glm::vec3   position;
     glm::vec3   rot_ori_position;
     ...
     GLfloat   delta;
     Mesh() :  id(0), rot_around_id(0), vbo_vertices(0), vbo_normals(0), ibo_elements(0), position(glm::vec3(0)), rot_ori_position(glm::vec3(0)), object2world(glm::mat4(1)), transform(glm::mat4(1)), rotation_mat(glm::mat4(1)), center(glm::vec3(0)), RotationAngle(0), delta(0) {}
     ~Mesh() {
	  if (vbo_vertices != 0)
     ...
     void logic()
	  {
	       Mesh* rot_mesh;
	       if( ((this->rot_around_id) > 0) && ((this->rot_around_id - 1) < num_obj) )
		     rot_mesh = mesh_objects[(this->rot_around_id)-1];
	       else
		    rot_mesh = this;
	       if (view_mode == MODE_OBJECT) { 
		    this->rot_ori_position  =  rot_mesh->position;
		    this->RotationAxisa     =  this->rot_ori_position; (this->RotationAxisa).y *= (-1.0f);

		    glm::mat4 Txyz = glm::translate(glm::mat4(1), (-1.0f)* this->rot_ori_position);

		    glm::vec3 RotationAxis = glm::vec3(this->RotationAxisa - this->rot_ori_position);

		    glm::mat4 TxyzInv = glm::translate(glm::mat4(1), this->rot_ori_position);
		    if(glm::abs(this->RotationAngle) < 360)
		    {
			 this->RotationAngle +=this->delta;
			 ...
			 this->object2world = this->rotation_mat * this->object2world;
			 	 this->position = glm::vec3( (this->object2world) * glm::vec4((this->position), 1.0f) );
		    }
		    else{
...
void load_obj(const char* filename, Mesh* mesh)
{
     ...
     in.close();
     if( (mesh->rot_around_id == mesh->id))
       {
	 mesh->rot_around_id = 0;
       }
     ...
     mesh->transform  = glm::translate(glm::mat4(1), (-1.0f)*mesh->position) * glm::scale(glm::mat4(1), size) * glm::translate(glm::mat4(1), (-1.0f)*mesh->center);
     mesh->object2world = mesh->transform * mesh->object2world;
}
...
int init_resources(char** model_filename, char* vshader_filename, char* fshader_filename)
{
     ...
     while(i< num_obj)
     {
       ...
       cout<<"(this->rot_around_id) "<<((mesh->rot_around_id) )<<endl;
       cout<<"(this->rot_around_id)-1 "<<((mesh->rot_around_id) - 1)<<endl;
       cout<<"\tposition :"<<mesh->position.x<<", "<<mesh->position.y<<", "<<mesh->position.z<<endl;
       i++;
     }
