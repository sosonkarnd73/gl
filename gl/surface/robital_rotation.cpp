...
public:  
     ...
     Mesh** 	 childs;
     Mesh* 	 parent;
     Mesh() :  id(0), numChilds(0), vbo_vertices(0), vbo_normals(0), ibo_elements(0), position(glm::vec3(0)), object2world(glm::mat4(1)), revolve(glm::mat4(1)), rotation_mat(glm::mat4(1)), delta(0), revolutionAngle(0) {}
     ~Mesh() {
	  if (vbo_vertices != 0)
     ...
     void logic()
	  {
	       ...
	       cout<<"id "<<(this->id)<<"\tposition : "<<(this->position.x)<<(this->position.y)<<(this->position.z)<<endl;
	       int i = 0;
	       if( this->childs != NULL )
		 while (  (i < this->numChilds) && (this->childs[i] != NULL) )
		    {
			 (this->childs[i])->logic();
			 i++;
		    }
     ...
     int load_obj(char** filename, Mesh* p) 
	  {
	       cout<<"file name "<<filename[0]<<endl;
	       ...
	       while(i < (this->numChilds)){
		    ...
		    if ((this->childs[i])->load_obj(&filename[i+1], this) == 0)
		    {
			 return 0;
		    }
		    i++;
	       }
	       cout<<"file name "<<filename[0]<<"return 1"<<endl;
	       return 1;
	  }
};
...
{
     if( mesh_object->load_obj(model_filename, NULL) == 0) return 0;
     mesh_object->upload();
