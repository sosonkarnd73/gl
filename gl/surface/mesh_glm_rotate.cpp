...
public:
     ...
     GLfloat   delta_rotZ;
     GLfloat   delta;
     Mesh() :  vbo_vertices(0), vbo_normals(0), ibo_elements(0),  object2world(glm::mat4(1)), transform(glm::mat4(1)), rotation_mat(glm::mat4(1)), euler_rotation(0), quat_rotation(0) , RotationAxisa(glm::vec3(1,0,0)), RotationAxisb(glm::vec3(1,0,0)), RotationAngle(0),  delta_rotX(0), delta_rotY(0), delta_rotZ(0), delta(5) {}
     ~Mesh() { 
	  if (vbo_vertices != 0)
     ...
     void logic()
	  {
	       ...
	       if (view_mode == MODE_OBJECT) {
		    ...
		    TxyzInv[3] = glm::vec4((this->RotationAxisb), 1.0f);
		     if(this->euler_rotation) 
		    {			 
			 this->rotation_mat = glm::rotate(this->rotation_mat, delta, glm::normalize(RotationAxis));
     ...
     void draw()
	  {
	       ...
	       glm::mat4 m = this->object2world * this->transform;	  
...
void onKey(unsigned char key, int x, int y)
{
     switch (key) {
     case 'x' :
	  if(main_object.euler_rotation)
	  {
	       main_object.delta_rotX = 1;
	  }
	  break;
     case 'y' :
	  if(main_object.euler_rotation)
	  {
	       main_object.delta_rotY = 1;
	  }
	  break;
     case 'z' :
	  if(main_object.euler_rotation)
	  {
	       main_object.delta_rotZ = 1;
	  }
	  break;
     case 'a' :
...
void onKeyUP(unsigned char key, int x, int y)
{
     ...
     case 'x' :
	  if(main_object.euler_rotation)
	  {
	       main_object.delta_rotX = 0;
	  }
	  break;
     case 'y' :
	  if(main_object.euler_rotation)
	  {
	       main_object.delta_rotY = 0;
	  }
	  break;
