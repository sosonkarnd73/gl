...
*/
double approx_length(int i1, int i2, double* points, int n);
...
double approx_length(int i1, int i2, double* points, int n){
     ...
     for(int j=0; j+4<=(n); j+=2){
	  d = distance(points[j], points[j+1], points[j+2], points[j+3]);
	  l+=d;
	  if(j>=i1 && (j)<=i2){
	       l2+=d;
	  }
...
int* getvert(GLuint* vbo)
{
     ...
     */
     ...
     /*
      * i1 and i2 are index of points on curve
      ...
      */
     double  length = 0.0;
     int i1 = (int)abc, i2 = (int)l[1]-2;
     scanf("%d%d",&i1,&i2);
     length = approx_length(i1, i2, points, l[1]);
