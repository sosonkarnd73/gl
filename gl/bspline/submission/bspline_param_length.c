...
double approx_length(double x1, double y1, double x2, double y2, double* points, int n){
     int a = 0, b = 0, j = 0;
     double l1 = 0.0, l2 = 0.0, d1 = 0.0, d2 = 0.0, d3 = 0.0, e = 0.0001, md1 = DBL_MAX, md2 = DBL_MAX, md3 = DBL_MAX,p1x,p1y,p2x,p2y;
     
     for(j=0; j+4<=(n); j+=2){
	  d1 = distance(points[j], points[j+1], x1, y1); 
	  d2 = distance(points[j], points[j+1], points[j+2], points[j+3]);
	  d3 = distance(points[j], points[j+1], x2, y2);
	  if(d1 < md1){
	       md1 = d1;
	       p1x = points[j];
	       p1y = points[j+1];
...
int* getvert(GLuint* vbo)
{
     ...
     */
     ...
     i = i-2;
     /*
     printf("Knot Span [%lf, %lf)\n",k_v[p],u);
     printf("First and Last Points on curve are: \n\t(%lf, %lf), (%lf,%lf)\n", points[0], points[1], points[i], points[i+1]);
     */
     /* u1 u2
      * since all the points are in single array i1 and i2 have to be even integers 
      * which will correspond to x coordinate of point they refer.
      */

     double  length = 0.0, u1 = k_v[p], u2 = u-e, search_points[4];
     scanf("%lf%lf",&u1,&u2);

     i = 0;
     b_spline_curve(p, u1, k_v, c_p, c_p_n, search_points, i);
     i+=2;
     b_spline_curve(p, u2, k_v, c_p, c_p_n, search_points, i);
     
     length = approx_length(search_points[0], search_points[1], search_points[2], search_points[3], points, l[1]);

     
     printf("Knots : %lf\t%lf\n",u1,u2);     
     printf("Distance Between Points corresponding to knots: \n\t(%lf, %lf), (%lf,%lf) :\tPercentage : %lf\n",search_points[0], search_points[1], search_points[2], search_points[3], length);
