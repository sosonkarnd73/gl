...
void onDisplay()
{
     ...
     glDrawArrays(GL_POINTS, 0, l[0]/2);	  
...
void free_resources()
{
     glDeleteBuffers(vbol,vbo);
     glDeleteBuffers(cbol,cbo);
...
int* getvert(GLuint* vbo)
{
     ...
     glBindBuffer(GL_ARRAY_BUFFER, vbo[1]); 
     glBufferData(GL_ARRAY_BUFFER, sizeof(points)*l[0], points, GL_STATIC_READ);
     glBindBuffer(GL_ARRAY_BUFFER, cbo[1]); 
     glBufferData(GL_ARRAY_BUFFER, sizeof(points_col)*l[1], points_col, GL_STATIC_READ);
